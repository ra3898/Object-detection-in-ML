...
# detection_mode = st.selectbox("Choose Detection Mode", ["Webcam Realtime Attendance Fill", "Other"])

# # Check the detection mode selected
# if detection_mode == "Webcam Realtime Attendance Fill":
#     subject = st.text_input("Enter Subject")
...
#         cam = cv2.VideoCapture(0)
#         frame_display = st.image([])  # Placeholder for webcam feed in Streamlit

#         face_detected_time = {}

#         # Attendance capturing loop
#         while cam.isOpened():
#             ret, img = cam.read()
#             if not ret:
...
#                     else:
#                         name = "Unknown - Not in Enrollment"

#                     # Mark attendance after 10 seconds
#                     if id not in face_detected_time:
#                         face_detected_time[id] = ts
...
#                         attendance.loc[len(attendance)] = [id, name, date, timeStamp, subject]
#                         st.info(f"Attendance marked for {name}")
#                         del face_detected_time[id]
#                 else:
#                     id = "unknown"
#                     confidence = f"{100 - confidence:.2f}%"

#                 cv2.putText(img, f"{id}", (x + 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
#                 cv2.putText(img, f"{confidence}", (x + 5, y + h - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 1)

#             # Convert frame to RGB for Streamlit
#             img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
#             pil_img = Image.fromarray(img_rgb)
#             frame_display.image(pil_img, caption="Webcam Feed", use_column_width=True)

#             # Save attendance to CSV periodically
#             ts = time.time()
#             date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
#             timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
#             file_name = f"Attendance/{subject}_{date}_{timeStamp.replace(':', '-')}.csv"
...
#             student_enrollments = set(df['Enrollment'].values)
#             attendance_enrollments = set(attendance['Enrollment'].values)
#             missing_enrollments = student_enrollments - attendance_enrollments

#             if missing_enrollments:
#                 st.error(f"Missing Enrollment IDs: {', '.join(map(str, missing_enrollments))}")
#             else:
#                 st.success("All students accounted for in attendance.")

#             # Update every 10 minutes (600 seconds)
#             time.sleep(600)

#             # Break loop with Streamlit stop button
#             if st.button("Stop Attendance"):
#                 break

#         cam.release()
#         st.write("Attendance session ended.")
if detection_mode == "Webcam Realtime Attendance Fill":
    subject = st.text_input("Enter Subject")
    current_time = time.time()
